...
- [ ] **Step 3b: Cerrar el contrato de `materialRelieve`**

La Task 5 dejó `indices` y `texelIndices` **opcionales** en `OpcionesRelieve`, y no le quedaba otra: si los hubiera puesto obligatorios, `tsc` se habría puesto rojo en este mismo `TerrainLod.tsx` hasta que llegara esta tarea. Ahora que ya los pasa, se cierran.

En `src/scene/terrainShader.ts`, quita los `?` de los dos campos de `OpcionesRelieve` y con ellos los `??` que los suplían:
...
Por qué obligatorios y no opcionales con respaldo: `texelIndices ?? 0` hace que `difiere()` muestree CUATRO VECES EL MISMO TEXEL, así que nunca encuentra una diferencia y la capa de límites no dibuja nada — sin error, sin aviso, sin nada que mirar. Un fallo silencioso es justo la clase de defecto que este proyecto ya pagó caro. Que el tipo lo exija cuesta tres líneas y lo vuelve imposible.

`tsc` va a señalar las tres llamadas que se quedan cortas. Son de prueba y no les importan los límites, así que se les pasa una textura vacía:

- `src/scene/terrainShader.test.ts`, las dos llamadas sueltas dentro de los `it` de ganancia y de cascadas (la helper `mat` ya los pasa).
- `src/foto/hypso.test.ts`, su llamada dentro del helper que compila el shader.

A las tres se les añade `indices: new THREE.Texture(), texelIndices: 1 / 2048`.
...
describe('validarCapa', () => {
  ...
  // sin esto, la recursión de `puntos()` no la ejecuta ninguna prueba, y la
  // rama que recorre los anillos de un polígono queda sin cubrir.
...
- Produces: `CapaPuntos({ capa, grid, meta, onElegir }: { capa: Capa; grid: Int16Array; meta: TerrainMeta; onElegir: (capa: Capa, rasgo: Rasgo) => void })`.

`onElegir` devuelve la capa además del rasgo: la ficha de la Task 12 necesita el catálogo de la capa para saber cómo se llama cada campo, y un rasgo no lo sabe. Buscar la capa después por el `id` del rasgo sería adivinar.
 ...
 * de la cámara, así que aunque la capa traiga cientos -- hospitales trae 127 --
 * son unos pocos raycasts por segundo.
 *
...
Añade el estado y los imports de `CAPAS`, `CapaPuntos`, `Capa` y `Rasgo`:
...
y usa `DISPONIBLES` en las **dos** llamadas que traducen la URL: el `useState` inicial y el
`useEffect` que la escribe. Las dos tienen que ver el catálogo COMPLETO o la capa de hospitales
no se puede prender desde un enlace, y `tsc` no lo atrapa porque `CAPAS_FIJAS` también es un
`Disponible[]` válido: hay que comprobarlo a mano.

Las filas del panel NO usan `DISPONIBLES`: necesitan `{ id, nombre, color? }` y `CapaFija` no
tiene color, así que van con el spread explícito de arriba.
...
El estado `elegido` ya existe desde la Task 11 y trae la capa junto al rasgo, así que esto solo monta. En `src/App.tsx`, junto a los demás paneles fijos (después de `<MapControls …/>`):
...
```bash
git add src/ui/FichaRasgo.tsx src/ui/FichaRasgo.test.tsx src/App.tsx
...
**3. Consistencia de tipos.** `Disponible` (Task 1) es estructural y `CapaFija` (Task 1) y `Capa` (Task 8) lo cumplen las dos. `Fila` del panel (Task 2) se arma en `App` desde ambas (Tasks 3, 11). `alturaGruesa` (Task 7) tiene la misma firma en `disco.ts` y en `CapaPuntos`. `UniformsRelieve` y `OpcionesRelieve` (Task 5) se consumen en `TerrainLod` (Task 6) con los nombres `indices` y `texelIndices`. `onElegir` de `CapaPuntos` nace ya con su firma final `(capa, rasgo)` en la Task 11, y la Task 12 solo la consume: ninguna tarea reescribe lo que otra acaba de escribir.
