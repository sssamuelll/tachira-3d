...
test('cada segmento puede tener normales propias aunque comparta el vértice de una curva', () => {
  const { nrm } = packRoads([{ enu: [[0, 0, 0], [10, 0, 1], [10, 10, 2]],
    nrmTramos: [[[0, 1, 0], [0, 1, 0]], [[0.6, 0.8, 0], [0.6, 0.8, 0]]] }])
  expect(Array.from(nrm)).toEqual([0, 127, 0, 0, 127, 0, 76, 102, 0, 76, 102, 0])
})
...
test('ocultar un túnel conserva su índice de vía sin geometría ni normales ni picking', () => {
  const visible = { enu: [[10, 20, 30], [40, 50, 60]], nrm: [[0.6, 0.8, 0], [0, 1, 0]] }
  const hidden = { hidden: true, enu: [[1, 2, 3], [4, 5, 6], [7, 8, 9]] }
  const p = packRoads([hidden, visible, hidden, visible, hidden])
  expect(p.segmentCount).toBe(2)
  expect(Array.from(p.index)).toEqual([0, 0, 1, 1, 2, 2])
  expect(Array.from(p.segIds)).toEqual([1, 3])
  expect(Array.from(p.positions)).toEqual([10, 30, -20, 40, 60, -50, 10, 30, -20, 40, 60, -50])
  expect(Array.from(p.nrm)).toEqual([76, 102, 0, 0, 127, 0, 76, 102, 0, 0, 127, 0])
})

test('un conjunto solo de túneles conserva un CSR válido sin reservar segmentos', () => {
  const p = packRoads([{ hidden: true, enu: [[0, 0, 0], [1, 0, 0]] }])
  expect(Array.from(p.index)).toEqual([0, 0])
  expect(p.segmentCount).toBe(0)
  expect(p.positions).toHaveLength(0)
  expect(p.segIds).toHaveLength(0)
  expect(p.nrm).toHaveLength(0)
})
